...
const PlatformWebView = React.forwardRef<any, PlatformWebViewProps>((props, ref) => {
  ...
  const handleError = (syntheticEvent: any) => {
    ...
    if (__DEV__) console.error('[PlatformWebView] WebView error:', nativeEvent);
  ...
  const renderError = (errorDomain: string | undefined, errorCode: number, errorDesc: string) => {
    return (
      <View style={nativeStyles.errorContainer}>
        <Text style={nativeStyles.errorTitle}>Connection Error</Text>
        <Text style={nativeStyles.errorMessage}>
  ...
  return (
    <View style={{ flex: 1 }}>
      {showErrorBanner && (
        <Animated.View style={[nativeStyles.errorBanner, { opacity: bannerOpacity, width: width - 40 }]}>
          <TouchableOpacity onPress={dismissErrorBanner} style={nativeStyles.bannerTouchable}>
            <Text style={[nativeStyles.bannerText, fontsLoaded && { fontFamily: 'Poppins-Regular' }]}>
...
const nativeStyles = StyleSheet.create({
  ...
  bannerText: {
    color: '#DC2727',
    fontSize: Math.min(Dimensions.get('window').width * 0.05, 24),
    fontWeight: '400',
    textAlign: 'center',
    lineHeight: Math.min(Dimensions.get('window').width * 0.065, 32),
  },
  errorContainer: {
    flex: 1,
    justifyContent: 'center',
