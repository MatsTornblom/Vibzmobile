 ...
 * 2. vibzworld://lovenote.vibz.world/shareId (fallback)
...
function extractTargetUrl(incomingUrl: string): string | null {
  try {
    ...
    // Format 2: Direct web link (App Links/Universal Links)
    ...
    // Format 3: Custom scheme with domain path (FALLBACK)
...
export function useDeepLinking() {
  ...
  useEffect(() => {
    ...
    const handleInitialURL = async () => {
      ...
      // Check for deferred deep link first (from AppsFlyer)
      ...
      // Check for initial URL from OS (cold start)
