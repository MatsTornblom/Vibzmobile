...
export function useAppsflyerDeepLinking() {
  ...
  const handleDeepLinkNavigation = useCallback(async (data: AppsflyerDeepLinkData) => {
    ...
    Alert.alert(
      ...
      `Extracted values:\n` +
      `- deep_link_value: ${data.deep_link_value || 'NOT FOUND'}\n` +
      `- af_dp: ${data.af_dp || 'NOT FOUND'}\n` +
      `- campaign: ${data.campaign || 'N/A'}\n` +
      `- media_source: ${data.media_source || 'N/A'}`,
      [{ text: 'OK' }]
    );
    ...
    if (!deepLinkValue) {
      ...
      Alert.alert(
        '⚠️ No Deep Link Value',
        'Could not find deep_link_value or af_dp in the data',
        [{ text: 'OK' }]
      );
      return;
    }
    ...
    if (!isFirstLaunch) {
      ...
      Alert.alert(
        '✅ First Launch - URL Stored',
        `Stored deferred deep link:\n\n${fullUrl}\n\nThis will be used on next app start.`,
        [{ text: 'OK' }]
      );
    } else {
      if (__DEV__) {
